...
public class ImageProcessing {
	...
	public boolean checkArguments(String[] args) {
		if (args.length != 2) { // Configuration file and output directory not specified
			...
			// Obtain configuration file and output directory
	...
	public void processImage() {
		...
		String separator; // Symbol used by the operating system to separate folders ina file path
		OS = System.getProperty("os.name");
		if (OS.startsWith("Win")) separator = "\\"; else separator = "//";
		try {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					if (loader.loadImage(parameters[1])) {
						...
						writer.writeImage(outputDir + separator + outputFile, result);
						++ filesProcessed;
					}
				}
				br.close();
				if (filesProcessed == 1) 
					System.out.println("Image file \""+outputDir+separator+outputFile+ "\" saved");
